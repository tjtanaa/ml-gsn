...
def create_json(train_scheme, train, fullposeslidar_file, lidar2cam_file, save_folder, org_json, filename='cameras.json'):
    f =open(org_json)
    data = json.load(f)
    f.close()
    content = []
    ...
    for i in range(0, 300):
        # K = np.array(data[i]['K'], dtype='float')
        # # print('Original K: ')
        # # print(K)
        # # (x, y ,z) -> (y, -z, x)
        # K[1,:] = -K[1,:]
...
def preprocess_imgs(train_scheme, new_folder_list, org_folder_list):
    imgs_0 = sorted(glob.glob('/disk2/kaho/projects_on_disk2/nerf/v1ml-gsn_s10_newseq_vKEmat/data/carla_stereo/rgb_L'+'/*_rgb.png'))
    depths_0 = sorted(glob.glob('/disk2/kaho/projects_on_disk2/nerf/v1ml-gsn_s10_newseq_vKEmat/data/carla_stereo/depth_L'+'/*_depth.png'))
    ...
    if train_scheme==8:
        ...
        imgs_0 = sorted(glob.glob('/disk2/kaho/projects_on_disk2/nerf/v1ml-gsn_s10_newseq_vKEmat/data/carla_stereo/rgb_L'+'/*_rgb.png'))
        depths_0 = sorted(glob.glob('/disk2/kaho/projects_on_disk2/nerf/v1ml-gsn_s10_newseq_vKEmat/data/carla_stereo/depth_L'+'/*_depth.png'))
        imgs_1 = sorted(glob.glob('/disk2/kaho/projects_on_disk2/nerf/v1ml-gsn_s10_newseq_vKEmat/data/carla_stereo/rgb_R'+'/*_rgb.png'))
        depths_1 = sorted(glob.glob('/disk2/kaho/projects_on_disk2/nerf/v1ml-gsn_s10_newseq_vKEmat/data/carla_stereo/depth_R'+'/*_depth.png'))
        assert (len(imgs_0)==562)
        assert (len(depths_0)==562)
        assert (len(imgs_1)==562)
        assert (len(depths_1)==562)
        
        imgs_train_0, depths_train_0, imgs_train_1, depths_train_1, imgs_test_0, depths_test_0, imgs_test_1, depths_test_1 = [],[],[],[],[],[],[],[]
        
        imgs_train_0 = imgs_0[0:300:1]
        depths_train_0 = depths_0[0:300:1]
        imgs_train_1 = imgs_1[0:300:1]
        depths_train_1 = depths_1[0:300:1]
        assert (len(imgs_train_0)==300)
        assert (len(depths_train_0)==300)
        assert (len(imgs_train_1)==300)
        assert (len(depths_train_1)==300)
        
        org_json_L = '/disk2/kaho/projects_on_disk2/nerf/v1ml-gsn_s10_newseq_vKEmat/data/carla_stereo/cameras_L.json'
        org_json_R = '/disk2/kaho/projects_on_disk2/nerf/v1ml-gsn_s10_newseq_vKEmat/data/carla_stereo/cameras_R.json'
        ...
        # TODO create_json
        res = create_json(train_scheme, True, org_folder_list[3], org_folder_list[-2], new_folder_list[-4], org_json_L)
        
        for i in range(len(imgs_train_1)):
            img = cv2.imread(imgs_train_1[i])#, cv2.IMREAD_UNCHANGED)
            depth = cv2.imread(depths_train_1[i])
            img = cv2.resize(img,  (480, 480), interpolation = cv2.INTER_AREA)
            depth = cv2.resize(depth,  (480, 480), interpolation = cv2.INTER_AREA)
            cv2.imwrite(os.path.join(new_folder_list[-2], str(i).zfill(3)+'_rgb.png'), img)
            cv2.imwrite(os.path.join(new_folder_list[-2], str(i).zfill(3)+'_depth.png'), depth)
        # TODO create_json
        res = create_json(train_scheme, True, org_folder_list[3], org_folder_list[-1], new_folder_list[-2], org_json_R)
            
        imgs_test_0 = imgs_0[0:300:1]
        depths_test_0 = depths_0[0:300:1]
        imgs_test_1 = imgs_1[0:300:1]
        depths_test_1 = depths_1[0:300:1]
        assert (len(imgs_test_0)==len(depths_test_0))
        print('len(imgs_test_0): ', len(imgs_test_0))
        ...
        # TODO create_json
        res = create_json(train_scheme, False, org_folder_list[3], org_folder_list[-2], new_folder_list[-3], org_json_L)
        
        for i in range(len(imgs_test_1)):
            img = cv2.imread(imgs_test_1[i])#, cv2.IMREAD_UNCHANGED)
            depth = cv2.imread(depths_test_1[i])
            img = cv2.resize(img,  (480, 480), interpolation = cv2.INTER_AREA)
            depth = cv2.resize(depth,  (480, 480), interpolation = cv2.INTER_AREA)
            cv2.imwrite(os.path.join(new_folder_list[-1], str(i).zfill(3)+'_rgb.png'), img)
            cv2.imwrite(os.path.join(new_folder_list[-1], str(i).zfill(3)+'_depth.png'), depth)
        # TODO create_json
        res = create_json(train_scheme, False, org_folder_list[3], org_folder_list[-1], new_folder_list[-1], org_json_R)
...
if __name__ == '__main__':
    train_scheme = 10
    new_base_folder = '/disk2/kaho/projects_on_disk2/nerf/v1ml-gsn_s10_newseq_vKEmat/data'
    
    org_folder = '/disk2/kaho/projects_on_disk2/nerf/v1ml-gsn_s10_newseq_vKEmat/data/carla_stereo'
