...
class VizdoomDataset(Dataset):
    ...
    def get_trajectory_Rt(self):
        ...
        for idxstr in self.seq_idxs:
            ...
            with open(os.path.join(episode_path, 'cameras.json'), 'r') as f:
                ...
                for i in range(0, self.episode_len +0):
    ...
    def __getitem__(self, idx):
        ...
        if self.samples_per_epoch:
            ...
            # Choose random start point (first ~50 frames have no movement) changed to 0 coz kiktti carla moves
            seq_start = random.randint(0, self.episode_len + 0 - (self.seq_len * self.step)) # seq_len*step=4*10=40, 300 images, 
        else:
        ...
        for idx, i in enumerate(range(seq_start, seq_start + (self.seq_len * self.step), self.step)):
            ...
            if self.depth:
                ...
                _depth = Image.open(_depth)                                
                _depth = self.resize_transform_depth(_depth)
                _depth = _depth.convert("RGB")
                _depth = np.array(_depth, dtype='float')
                
                _depth = (_depth[:,:,0] + _depth[:,:,1]*256 + _depth[:,:,2]*256*256 ) / ( 256*256*256 - 1 )
                depth.append(torch.from_numpy(_depth).unsqueeze(0))
